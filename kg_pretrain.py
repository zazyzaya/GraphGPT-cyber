...
def train(g: RWSampler, model: BERT):
    ...
    model.eval()
    te_metrics, va_metrics = evaluator.get_metrics(g,va,te,model)
    with open(f'{OUT_F}eval_{SIZE}.csv', 'w+') as f:
        f.write('e,te_mmr,te_hits@1,te_hits@5,te_hits@10,va_mmr,va_hits@1,va_hits@5,va_hits@10,\n')
    ...
    while processed_tokens < TOTAL_T:
        ...
        with torch.no_grad():
            if e % EVAL_EVERY == 0: 
                ...
                model.eval()
                va_scores, te_scores = evaluator.get_metrics(g, va, te, model)
                ...
                with open(f'{OUT_F}eval_{SIZE}.csv', 'a') as f:
                    f.write(f'{updates},{",".join([str(s) for s in te_scores])},{",".join([str(s) for s in va_scores])}\n')
