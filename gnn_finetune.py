...
from models.gnn_bert import GNNBert, GNNEmbedding
...
def eval_causal(model, tr, to_eval):
    ...
    for i in range(NUM_EVAL_ITERS):
        ...
        for idx in idxs:
            ...
            out = model.modified_fwd(tr.edge_index, walk, mask, tgt, None, return_loss=False).logits
...
def train_causal(tr,va,te, model: GNNBert):
    opt = AdamW(
    ...
    for e in range(EPOCHS):
        ...
        for idx in idxs:
            ...
            loss = model.modified_fwd(tr.edge_index, walk, mask, tgt, None)
            loss.backward()
...
if __name__ == '__main__':
    ...
    model = GNNBert(config)
    sd = torch.load(f'pretrained/static_graph/rw_sampling/gnn_bert_{SIZE}.pt', weights_only=True)
