...
from models.gnn_bert import RWBertFT, RWBert, GNNEmbedding
...
def sample(edges, batch, walk_len=3):
    starts = batch.view(-1)
    rw = random_walk(edges[1], edges[0], starts, walk_len)

    src,dst = rw.chunk(2)
    src = src.flip(1)
    walks = torch.cat([src,dst], dim=1)

    return walks

def sample_causal(edges, batch, walk_len):
    starts = batch[0]
    rw = random_walk(edges[1], edges[0], starts, walk_len).flip(1)
    masks = torch.tensor([[GNNEmbedding.MASK]]).repeat(rw.size(0),1)
    rw = torch.cat([rw,masks], dim=1)

    return rw, rw==GNNEmbedding.MASK, batch[1]

@torch.no_grad()
def eval_causal(model, tr, to_eval):
    ...
    for i in range(NUM_EVAL_ITERS):
        ...
        for idx in idxs:
            walk, mask, tgt = sample_causal(tr.edge_index, to_eval.edge_index[:, idx], WALK_LEN)

            out = model.modified_fwd(walk, mask, tgt, None, return_loss=False).logits
            out = 1 - torch.softmax(out, -1)
...
def eval(model, tr, to_eval):
    ...
    for i in range(NUM_EVAL_ITERS):
        walks = sample(tr.edge_index, to_eval.edge_index)
        idxs = torch.arange(walks.size(0)).split(EVAL_BS)

        for idx in idxs:
            edges = walks[idx]
            pred = torch.sigmoid(model.predict(edges)).to('cpu')
            preds[idx] += pred.squeeze()

    preds /= NUM_EVAL_ITERS

    wl = (to_eval == WHITELIST).sum(dim=0, dtype=torch.bool)
    preds[wl] = 0
...
def train_causal(tr,va,te, model: RWBert):
    opt = AdamW(
    ...
    with open(f'{HOME}/ft_results_{SIZE}_wl{WALK_LEN}.txt', 'w+') as f:
    ...
    for e in range(EPOCHS):
        ...
        for idx in idxs:
            model.train()
            walk,mask,tgt = sample_causal(tr.edge_index, tr.edge_index[:,idx], WALK_LEN)
            loss = model.modified_fwd(walk, mask, tgt, None)
            loss.backward()
        ...
        model.eval()
        add_fake_data(va)
        auc, ap = eval_causal(model, tr, va)
        print('#'*20)
        print(f'VAL SCORES')
        print('#'*20)
        ...
        auc, ap = eval_causal(model, tr, te)
        print('#'*20)
        print(f'TEST SCORES')
        print('#'*20)
        print(f"AUC: {auc:0.4f}, AP:  {ap:0.4f}")
        ...
        with open(f'{HOME}/ft_results_{SIZE}_wl{WALK_LEN}.txt', 'a') as f:
...
def train(tr,va,te, model: RWBertFT):
    ...
    best_te = None
    for e in range(EPOCHS):
        walks = sample(tr.edge_index, tr.edge_index, walk_len=WALK_LEN)
        for pos_walks in walks.split(MINI_BS):
            model.train()

            # Negative sample non-edges
            neg = torch.randint(0, tr.x.size(0), (pos_walks.size(0),2))
            neg_walks = sample(tr.edge_index, neg, walk_len=WALK_LEN)
            labels = torch.zeros(pos_walks.size(0)+neg.size(0), 1)
            labels[pos_walks.size(0):] = 1

            walk = torch.cat([pos_walks,neg_walks])
            loss = model.forward(walk, labels)
            loss.backward()
...
if __name__ == '__main__':
    ...
    model = RWBert(config)
    sd = torch.load(f'pretrained/static_graph/rw_sampling/bert_{SIZE}.pt', weights_only=True)
    model.load_state_dict(sd)
    model = model.to(DEVICE)
    train_causal(tr,va,te, model)

    #model = RWBertFT(config, f'pretrained/static_graph/rw_sampling/bert_{SIZE}.pt', device=DEVICE)
