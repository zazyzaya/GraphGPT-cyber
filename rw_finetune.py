...
def train(tr,va,te, model: RWBert):
    ...
    with open(f'{HOME}/{DATASET}/rw-ft_results_{FNAME}_{SIZE}_wl{WALK_LEN}.txt', 'w+') as f:
    ...
    for e in range(EPOCHS):
        for samp in tr.edge_iter():
            ...
            if updates and updates % EVAL_EVERY == 0:
                ...
                with open(f'{HOME}/{DATASET}/rw-ft_results_{FNAME}_{SIZE}_wl{WALK_LEN}.txt', 'a') as f:
...
if __name__ == '__main__':
    ...
    tr = RWSampler(tr, device=DEVICE, walk_len=WALK_LEN, batch_size=MINI_BS, edge_features=edge_features)
