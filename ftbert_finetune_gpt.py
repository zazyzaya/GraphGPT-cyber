...
from models.hugging_gpt import GPT_Cls 
...
def parallel_eval(model, tr, te, workers=1):
    ...
    def thread_job(pid, idx):
        ...
        walk = sample(tr, src,dst,ts, WALK_LEN, edge_features=ef)
        out = model.predict(walk)
...
def train(tr,va,te, model: GPT_Cls):
    opt = AdamW(
...
if __name__ == '__main__':
    ...
    model = GPT_Cls(config, sd, device=DEVICE, from_random=args.from_random)
